...
Режим: **Operate**. Это внутренняя кафедральная рабочая система, где важнее скорость сканирования, предсказуемость и безопасные операции, чем визуальный ребрендинг. `DESIGN.md` намеренно не создаётся: визуальный мир, бренд и исходные референсы не подтверждены.
...
Семантические токены: `--color-canvas`, `--color-surface`, `--color-text`, `--color-text-muted`, `--color-border`, `--color-focus`, `--color-action`, `--color-danger`, `--color-warning`, `--color-success`; `--space-*`; `--radius-*`; `--shadow-*`; `--font-*`; `--z-*`; `--duration-*`. Конкретные палитра, шрифты, логотип и визуальный характер не фиксируются до решения владельца.
...
4. Когда будут подтверждены название/бренд/визуальные референсы: отдельное решение о visual world и только затем `DESIGN.md`.
